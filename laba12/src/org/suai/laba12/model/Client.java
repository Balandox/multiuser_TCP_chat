...
/*
на клиете по прежнему работает два потока, один для просулшки, другой для отправки
сервер просто тупа раздает сокеты и запускаем класс ClientHandler, который уже в свою очередь занимается рассылкой ответа от сервера
ClientHandler слушает сообщение от своего клиента и рассылает их либо всем, либо определенному челу
т.е. Client будет слушать сообщения от сервера,
 */
